...
fn setup_elements(font: &Font) -> Element {
    let elements = {
        use Element::*;
        Stack(vec![
            // Top bar.
            Row(vec![Text("gemini://example.com/".to_string())]),
            // Main content.
            Stack(vec![
                Paragraph(WrappedText::new(LOREM, 400, font), 400, 200),
                Row(vec![
                    Text("=>".to_string()),
                    Space,
                    Paragraph(
                        WrappedText::new("gemini://geminiprotocol.net/", 450, font),
                        450,
                        16,
                    ),
                ]),
