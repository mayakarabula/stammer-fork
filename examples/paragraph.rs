...
use stammer::elements::{Alignment, Content, Element, SizingStrategy, WrappedText};
use stammer::Panel;
...
fn setup_elements(font: Rc<Font>) -> Element<Data> {
    fn resize_width(element: &mut Element<Data>, data: &Data) {
        ...
        let Content::Stack(children) = &mut element.content else {
            unimplemented!()
        };

        for child in children {
            let Content::Paragraph(wrapped, _) = &mut child.content else {
                unimplemented!()
            };

            wrapped.rewrap(Some(data.width), &element.style.font)
        }
    }
    ...
    Element::dynamic(
        ...
        Stack(vec![
            Element::still(
                Rc::clone(&font),
                Paragraph(
                    WrappedText::new(LOREM.to_string(), 420, &font),
                    Alignment::Left,
                ),
            )
